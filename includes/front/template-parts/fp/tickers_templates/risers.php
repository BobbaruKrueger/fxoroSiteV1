<div class="item R<?php echo $result->symbol;?>">
        ...
        <div class="col-10 offset-1 col-sm-12 offset-sm-0 itemInner">
            ...
            <div class="row">
                <div class="col-12 currencyW">
                    <?php echo $result->name; ?> <i class="fa fa-chevron-up fa-xs" style="color:green;"></i>
                </div>
            </div>
            <div class="row">
            ...
            <div class="row">
                <div class="col-12 canvasW">
                    ...
                    <?php if($result->category =="FOREX"): ?>
                        <canvas id="R<?php echo $result->symbol;?>"></canvas>
                    <?php else: ?>
                        <canvas id="R<?php echo str_replace( ' ',  '',$result->name);?>"></canvas>
                    <?php endif; ?>
                </div>
            </div>
            <div class="row">
...
<script>
    <?php if ( $result->category =="FOREX" ):?>
        let Rlastfifteen<?php echo $result->symbol; ?> = <?php echo json_encode($last_fifteen_values);?>;
        let Rselector<?php echo $result->symbol; ?> = document.getElementById("R<?php echo $result->symbol; ?>").getContext('2d');
        for (let x = 0; x < <?php echo count($first_fifteen_values) -1 ; ?>; x++) {

            Rlastfifteen<?php echo $result->symbol; ?>.unshift( NaN);
        }
        let RaverageData<?php echo $result->symbol; ?> = []; 
        for (let x = 0; x < <?php echo count($dates_values); ?>; x++) {
            RaverageData<?php echo $result->symbol; ?>.push(<?php echo $average_all; ?>);
            
        }

        let Rchart<?php echo $result->symbol; ?> = new Chart(Rselector<?php echo $result->symbol; ?>, {
            ...
            data: {
                ...
                },
                {
                    ...
                    borderColor: 'rgba(218, 223, 225, 1)',
                    fill: false,
                    data: RaverageData<?php echo $result->symbol; ?>, 
                    type: 'line',
                    showLine: false
                },
                {
                    ...
                    fill: false,
                    data: Rlastfifteen<?php echo $result->symbol; ?>,
                    type: 'line',
                }]
            },        
    ...
    <?php else:?>
        let Rlastfifteen<?php echo str_replace( ' ',  '',$result->name); ?> = <?php echo json_encode($last_fifteen_values);?>;
        let Rselector<?php echo str_replace( ' ',  '',$result->name); ?> = document.getElementById("R<?php echo str_replace( ' ',  '',$result->name); ?>").getContext('2d');
        for (let x = 0; x < <?php echo count($first_fifteen_values) -1 ; ?>; x++) {

            Rlastfifteen<?php echo str_replace( ' ',  '',$result->name); ?>.unshift( NaN);
        }
        let RaverageData<?php echo str_replace( ' ',  '',$result->name); ?> = []; 
        for (let x = 0; x < <?php echo count($dates_values); ?>; x++) {
            RaverageData<?php echo str_replace( ' ',  '',$result->name); ?>.push(<?php echo $average_all; ?>);
            
        }

        let Rchart<?php echo str_replace( ' ',  '',$result->name); ?> = new Chart(Rselector<?php echo str_replace( ' ',  '',$result->name); ?>, {
            // The type of chart we want to create
            ...
            data: {
                ...
                },
                {
                    ...
                    borderColor: 'rgba(218, 223, 225, 1)',
                    fill: false,
                    data: RaverageData<?php echo str_replace( ' ',  '',$result->name); ?>, 
                    type: 'line',
                    showLine: false
                },
                {
                    ...
                    borderColor: '<?php echo $tendency_colour;?>',
                    fill: false,
                    data: Rlastfifteen<?php echo str_replace( ' ',  '',$result->name); ?>,
                    type: 'line',
                }]
            },        
            // Configuration options go here
