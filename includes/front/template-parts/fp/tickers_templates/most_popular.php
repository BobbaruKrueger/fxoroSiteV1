<div class="item MP<?php echo $result->symbol;?>">
        ...
        <div class="col-10 offset-1 col-sm-12 offset-sm-0 itemInner">
            ...
            <div class="row">
                <div class="col-12 canvasW">
                    ...
                    <?php if($result->category =="FOREX"): ?>
                        <canvas id="MP<?php echo $result->symbol;?>"></canvas>
                    <?php else: ?>
                        <canvas id="MP<?php echo str_replace( ' ',  '',$result->name);?>"></canvas>
                    <?php endif; ?>
                </div>
            </div>
            <div class="row">
...
<script>
    <?php if ( $result->category =="FOREX" ):?>
        let MPlastfifteen<?php echo $result->symbol; ?> = <?php echo json_encode($last_fifteen_values);?>;
        let MPselector<?php echo $result->symbol; ?> = document.getElementById("MP<?php echo $result->symbol; ?>").getContext('2d');
        for (let x = 0; x < <?php echo count($first_fifteen_values) -1 ; ?>; x++) {

            MPlastfifteen<?php echo $result->symbol; ?>.unshift( NaN);
        }
        let MPaverageData<?php echo $result->symbol; ?> = []; 
        for (let x = 0; x < <?php echo count($dates_values); ?>; x++) {
            MPaverageData<?php echo $result->symbol; ?>.push(<?php echo $average_all; ?>);
            
        }

        let MPchart<?php echo $result->symbol; ?> = new Chart(MPselector<?php echo $result->symbol; ?>, {
            ...
            data: {
                ...
                },
                {
                    ...
                    borderColor: 'rgba(218, 223, 225, 1)',
                    fill: false,
                    data: MPaverageData<?php echo $result->symbol; ?>, 
                    type: 'line',
                    showLine: false
                },
                {
                    ...
                    fill: false,
                    data: MPlastfifteen<?php echo $result->symbol; ?>,
                    type: 'line',
                }]
            },        
    ...
    <?php else:?>
        let MPlastfifteen<?php echo str_replace( ' ',  '',$result->name); ?> = <?php echo json_encode($last_fifteen_values);?>;
        let MPselector<?php echo str_replace( ' ',  '',$result->name); ?> = document.getElementById("MP<?php echo str_replace( ' ',  '',$result->name); ?>").getContext('2d');
        for (let x = 0; x < <?php echo count($first_fifteen_values) -1 ; ?>; x++) {

            MPlastfifteen<?php echo str_replace( ' ',  '',$result->name); ?>.unshift( NaN);
        }
        let MPaverageData<?php echo str_replace( ' ',  '',$result->name); ?> = []; 
        for (let x = 0; x < <?php echo count($dates_values); ?>; x++) {
            MPaverageData<?php echo str_replace( ' ',  '',$result->name); ?>.push(<?php echo $average_all; ?>);
            
        }

        let MPchart<?php echo str_replace( ' ',  '',$result->name); ?> = new Chart(MPselector<?php echo str_replace( ' ',  '',$result->name); ?>, {
            // The type of chart we want to create
            ...
            data: {
                ...
                },
                {
                    ...
                    borderColor: 'rgba(218, 223, 225, 1)',
                    fill: false,
                    data: MPaverageData<?php echo str_replace( ' ',  '',$result->name); ?>, 
                    type: 'line',
                    showLine: false
                },
                {
                    ...
                    borderColor: '<?php echo $tendency_colour;?>',
                    fill: false,
                    data: MPlastfifteen<?php echo str_replace( ' ',  '',$result->name); ?>,
                    type: 'line',
                }]
            },        
            // Configuration options go here
